...
\mtable[yshift=30\baselineskip]{Approximating $\int_0^4(4x-x^2)\dd x$ with the Right Hand Rule and 16 evenly spaced subintervals.}{fig:rie7}{\begin{tikzpicture}[alt={Blue parabola y=4x-x² on 0<x<4; sixteen narrow red rectangles use right endpoints, shading under curve; axes x and y.}]
...
&\iflatexml\quad\fi&\text{(by \autoref{thm:riemannSum}(2))}\\
&=\lim_{n\to\infty} 0\\
&=0
\end{align*}
...
&\iflatexml\quad\fi&\text{(\autoref{thm:riemannSum}(2))}\\
&=\lim_{n\to\infty} \sum_{i=1}^n f(c_i)(-\Delta x)\\
