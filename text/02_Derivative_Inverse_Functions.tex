...
%
%\mtable{A function $f$ along with its inverse $f\primeskip^{-1}$. (Note how it does not matter which function we refer to as $f$; the other is $f\primeskip^{-1}$.)}{fig:inverse1}{\begin{tikzpicture}[alt={Blue curve f and red inverse reflect over dashed y=x; blue passes (-0.5,0.375)→(1,1.5); red passes (0.375,-0.5)→(1.5,1).}]
