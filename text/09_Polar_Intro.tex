...
\smallskip\\(b)}
	...
	\item \begin{enumerate}
		...
		This is not the angle we desire. The range of $\tan^{-1}x $ is $(-\pi/2,\pi/2)$; that is, it returns angles that lie in the 1\textsuperscript{st} and 4\textsuperscript{th} quadrants. To find locations in the 2\textsuperscript{nd}  and 3\textsuperscript{rd} quadrants, add $\pi$ to the result of $\tan^{-1}x$. So  $\pi+(-\pi/4)$ puts the angle at $3\pi/4$. Thus the polar point is $(\sqrt{2},3\pi/4)$.
...
\label{fig:polar5table}\tagstructend
\end{center}
