...
\begin{center}
% otherwise, LaTeXML puts the first box much higher
\iflatexml
\fcolorbox{white}{white}{\parbox{\iflatexml75pt\else\widthof{input specific}\fi}{\centering input specific\\number \intertextmath{c}}}
\else
\parbox{\iflatexml75pt\else\widthof{input specific}\fi}{\centering input specific\iflatexml\else\\\fi number \intertextmath{c}}
\fi
$\longrightarrow$
\fbox{\parbox{\iflatexml80pt\else\widthof{do something}\fi}{\centering do something\\to \intertextmath{f} and \intertextmath{c}}}
$\longrightarrow$
\parbox{\iflatexml80pt\else\widthof{number $\fp(c)$}\fi}{\centering return\\number \intertextmath{\fp(c)}}
\end{center}
...
input variable $x$
$\longrightarrow$
\fbox{\parbox{\iflatexml80pt\else\widthof{do something}\fi}{\centering do something\\to \intertextmath{f} and \intertextmath{x}}}
$\longrightarrow$
\parbox{\iflatexml85pt\else\widthof{function $\fp(x)$}\fi}{\centering return\\function \intertextmath{\fp(x)}}
\end{center}
...
\begin{align*}
	...
	\intertext{Now find a common denominator and subtract; factor \intertextmath{1/h} out front to facilitate reading.}
	\fp(x)
...
Before applying \autoref{def:the_derivative}, note that once the derivative is found, we can find the actual tangent line to $f(x) = \sin x$ at $x=0$, whereas we settled for an approximation in \autoref{ex_der_num_approx}. 
	...
	\begin{align*}
		&\fp(x)\\
		&= \lim_{h\to 0} \frac{\sin(x+h)-\sin x}{h} & & \left(\text{\scriptsize \parbox{110pt}{\centering Use trig identity \intertextmath{\sin(x+h) = \sin x\cos h+\cos x\sin h}}}\right) \\
