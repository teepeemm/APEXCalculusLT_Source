...
\intertext{Now substitute, with \intertextmath{u=\tan x}, with \intertextmath{\dd u = \sec^2x\dd x}.}
...
\end{aligned}
\]
\caption{Setting up Integration by Parts.}\label{fig:trigint1}\tagstructend
\end{minipage}
...
\end{align*}
\end{example}
\mnote[yshift=13\baselineskip]{\textbf{Note:} Remember that in \autoref{ex_sub7}, we found that $\int\sec x\dd x=\ln\abs{\sec x+\tan x}+C$}
...
\intertext{We integrate the first integral with substitution, \intertextmath{u=\tan x} and \intertextmath{\dd u=\sec^2x\dd x}; and the second by employing rule \#4 again.}
