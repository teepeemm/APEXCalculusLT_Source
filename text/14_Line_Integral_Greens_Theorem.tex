...
\int_C \vec F\cdot \vec n\dd s &= \int_C \vec F\cdot \frac{\bracket{g'(t),-\fp(t)}}{\norm{\vrp(t)}} \norm{\vrp(t)}\dd t \\
				...
				\intertext{As the \intertextmathbegin$x$\intertextmathend{} and \intertextmathbegin$y$\intertextmathend{} components of \intertextmathbegin$\vrt$\intertextmathend{} are \intertextmathbegin$f(t)$\intertextmathend{} and \intertextmathbegin$g(t)$\intertextmathend{} respectively, the differentials of \intertextmathbegin$x$\intertextmathend{} and \intertextmathbegin$y$\intertextmathend{} are \intertextmathbegin$\dd x = \fp(t)\dd t$\intertextmathend{} and \intertextmathbegin$\dd y=g'(t)\dd t$\intertextmathend{}. We can then write the above integrals as:}
				&= \int_C M\dd y - \int_C N\dd x.\\
				\intertext{This is often written as one integral (not incorrectly, though somewhat confusingly, as this one integral has two ``\intertextmathbegin$\dd$\intertextmathend{}\,'s''):}
				&=\int_CM\dd y -N\dd x.
\end{align*}
...
\intertext{(where \intertextmathbegin$C_2^*$\intertextmathend{} is the path \intertextmathbegin$C_2$\intertextmathend{} traversed with opposite orientation)}
	&= \int_{C_1} \vec F\cdot \vec n\dd s -\int_{C_2} \vec F\cdot \vec n\dd s.\\
