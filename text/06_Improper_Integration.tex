...
%
\mtable[4\baselineskip]{A graph of $f(x) = \frac{1}{1+x^2}$ in \autoref{ex_impint1} part 4.}{fig:impint1d}{\begin{tikzpicture}[alt={Blue graph of f(x)=1/(1+x²), symmetric, peak y=1 at x=0, then falls to y=0 by x=10; area below curve to x-axis shaded.}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
...
\end{align*}
%
\mtable[-4\baselineskip]{A graph of $f(x)=\frac{1}{x^2}$ in \autoref{ex_impint3}.}{fig:impint3b}{\begin{tikzpicture}[alt={Blue graph f(x)=1/x² on −1<x<1 with a tall central spike (clipped at y=10) and symmetric flanks; full region under curve shaded.}]
\begin{axis}[width=1.16\marginparwidth,tick label style={font=\scriptsize},
axis y line=middle,axis x line=middle,name=myplot,axis on top,
