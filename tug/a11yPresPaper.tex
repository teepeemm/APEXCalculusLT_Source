...
\title{Creating accessible presentations with \texttt{ltx-talk}}
...
\NewDocumentCommand{\visablealert}{d<>m}{\visible<#1->{\alert<#1>{#2}}}
...
called \texttt{ltx-talk} with proper tagging support as a
...
A \texttt{ltx-talk} presentation must begin with
...
it is important to note that \texttt{ltx-talk} makes use of \emph{templates},
so that we will want to familiarize ourselves
with \texttt{texdoc lttemplates}.

Broadly, we can view a \texttt{ltx-talk} as a properly tagged \LaTeX\ document
...
\texttt{ltx-talk} defines three colors:
alert (red),
example (green),
and
structure (blue).
...
By default, \texttt{ltx-talk} sets frame titles to be \texttt{H4}.
...
Because \texttt{ltx-talk} has adapted the \texttt{beamer}
...
\vspace{-.5\baselineskip}\begin{verbatim}[\verbatimOptions]
\end{frame}
\end{verbatim}
...
\vspace{-1\baselineskip}\begin{verbatim}[\verbatimOptions]
\end{frame}
\end{verbatim}
%Because \texttt{ltx-talk} borrows its overlay concepts from \texttt{beamer},
The previous content works in \texttt{beamer} with the only necessary changes
being to remove the \texttt{tag-slides=all} and
...
\fbox{\includegraphics[page=5]{tugFigs}}%
\fbox{\includegraphics[page=6]{tugFigs}}
\fbox{\includegraphics[page=7]{tugFigs}}%
\fbox{\includegraphics[page=8]{tugFigs}}
\fbox{\includegraphics[page=9]{tugFigs}}%
\fbox{\includegraphics[page=10]{tugFigs}}
\caption{Output of example Matrix Multiplication code.}
\label{matrixMult}
\end{figure}

It is worthwhile to comment on the \texttt{tag-slides=all} argument.
% Joseph - is "all" documented?  Or not supposed to be used?
As we remarked with the previous example,
it is not useful to tag slides with slightly varying information.
Consequently, \texttt{ltx-talk} would normally only tag
the final slide in the frame.
Conceptually, we would prefer to have \texttt{ltx-talk} \emph{tag all slides}
in the frame.
In addition to taking an overlay specification,
\texttt{tag-slides} also recognizes
...
These are created in \texttt{ltx-talk} using templates,
...
color & structure \\
font & \cs{normalfont}\\
height & 12mm\\% top margin + headsp
left-hspace & 10mm\\% left margin
right-hspace & 10mm\\% right margin
print-frame-title & true\\\bottomrule
\end{tabular}
\end{table}
We can adjust the values by using
\cs{EditInstance}\tubbraced{header}\tubbraced{std}\tubbraced{\meta{key=value}}.
(Note that adjusting the font is probably better done with the
\texttt{frametitle} instance.)
...
\subsection{Adjusting the Titlepage}

The defaults for the \texttt{titlepage} template are in
\autoref{titlepageTemplateDefaults}.
\begin{table}
\caption{Default values of the \texttt{titlepage} type \texttt{talk} template}
\label{titlepageTemplateDefaults}
\centering
...
vertical-alignment & center\\\bottomrule
\end{tabularx}
\end{table}
We could \cs{EditTemplateDefaults}, but that is only necessary
if we plan to create multiple titlepages.
If we have a single titlepage, we can pass non-default values
...
we can also use \texttt{wallpaper} which remove the footer and the title
(but not the background color of the header).

\subsection{Creating a Custom Title Element}
...
\texttt{ltx-talk} will then use \cs{@myelement},
...
\DeclareInstance{footer-element}{myelement}
  {talk}{...}
\end{verbatim}
\texttt{ltx-talk} will then use \cs{@shortmyelement} or \cs{@myelement}
...
There are several more templates and instances in \texttt{ltx-talk},
...
\bottomrule
\end{tabularx}
\caption{Instances and types in the template \texttt{talk} in \texttt{ltx-talk}}
...
We may see information about these during compilation and in the log file
by using the template commands
\begin{itemize}
...
As \texttt{ltx-talk}'s reason for existence is proper tagging,
